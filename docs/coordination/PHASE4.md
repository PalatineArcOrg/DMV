...
## WP 4.8 side-by-side signing-identity architecture

WP 4.8 separates Android identity into:

```text
legacy_bridge
  package: com.romulusol.deadmansvault
  legacy EAS/Firebase/certificate only
  private devnet in-place bridge
  retains existing SecureStore and rollback key

successor
  required distinct package
  required distinct EAS/Firebase/certificate
  independent SecureStore/update/scheme identity
  devnet side-by-side migration app
```

Typed dynamic Expo configuration fails closed on missing or mixed identity,
validates Firebase package metadata and rejects mainnet. Source EAS profiles
are internal devnet APK profiles reserved for a later explicit artifact gate;
WP 4.8 does not execute them.

The incoming migration state is structured and restartable:

```text
legacy authority verified
→ successor candidate secured
→ candidate deliberately funded
→ candidate-payer + owner-signed rotation
→ canonical successor authority verified
→ separate deliberate successor heartbeat verified
→ deliberate notification register/decline
→ bridge retention
→ later cleanup gate
```

The successor needs only the prior agent public key from chain. It never
possesses or imports the prior secret. The successor key slot can promote a
canonically verified candidate from an intentionally empty active slot.

Rotation is not heartbeat proof because `rotate_agent` resets the timestamp but
does not increment `total_heartbeats`. Migration requires a later
successor-signed heartbeat whose verified count is strictly higher than the
pre-rotation count.

The retained bridge supports deliberate rollback by using its stored key as
replacement transaction payer plus owner instruction signer. Heartbeat remains
blocked on the bridge while its key is not current on-chain authority.

The cleanup predicate requires distinct package/certificate evidence,
restart-resolved successor custody, verified successor heartbeat and deadline,
accepted fee state, no unresolved operation, a notification decision, retained
bridge, disposable rollback evidence and captured evidence. Even then,
uninstall is a separate explicit gate.

Full architecture and rejected alternatives are recorded in
`docs/android-signing-identity-migration.md`.

Status: Source-only implementation. No package provisioning, Firebase app,
keystore, EAS command, APK, install, live rotation/heartbeat/notification,
program/IDL or mainnet action occurred.
