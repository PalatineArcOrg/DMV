...
**Status:** Final. Design vetted by a 6-agent review panel (2 rounds: security ×2,
correctness ×2, best-practices, integration). All decisions locked. Implement
directly from this document. Companion/history: `permissionless-execution-design.md`
(this file is the consolidated, build-ready version — prefer this one).

**Program:** Anchor 0.32.1 · Solana CLI 3.0.15 · Program ID
`GXCu5964mvgAJDWmcMriZpzU3vDVqPzjYCM1sxCnsoEb` (devnet).
...
- **Specific bequests (v1: SPL tokens + NFTs only):** an owner-defined `AssetPlan`
  assigns exact token amounts / whole NFTs to specific beneficiaries; those are
  carved out first, the remainder splits pro-rata. A beneficiary can receive a
  specific bequest **and** their share of the residual.
...
| D2 | Specific bequests are **SPL tokens + NFTs only** in v1. **Specific-SOL is cut to fast-follow** (avoids the Anchor runtime-conditional-accounts problem). SOL is always pure pro-rata. |
...
| D5 | On close: **rent → owner**; **token/SOL dust → largest-share beneficiary**. |
...
| D7 | Reduce `AGENT_FUNDING_LAMPORTS` 0.05 → **0.01 SOL** (agent only needs heartbeat fees; nothing refunds it on autonomous execution). |
...
Anyone cranks (server / beneficiary / keeper), reading on-chain masks to resume:
   begin_execution()                      # snapshot SOL residual = lamports - rent
   ...
   for each specific assignment j (in-order per mint):
     execute_specific_asset(j)            # exact token/NFT -> assignee
   ...
   close_vault()                          # masks full, no ATAs -> close core PDAs + AssetPlan, rent->owner
```
...
- Beneficiary becomes `{ wallet: Pubkey, share_bps: u16 }`. Shares still sum to 10000
  (validated at `initialize_vault`). Update `SPACE`.
...
pub struct ExecutionLog {
    pub vault: Pubkey,                 // 32
    pub sol_snapshot: u64,             // 8   lamports residual frozen at begin_execution
...
pub struct AssetAssignment {
    pub mint: Pubkey,             // 32  (SPL/NFT only in v1)
...
for `is_nft` pass+check `Mint.decimals==0 && supply==1`; no duplicate (mint,benef) you
disallow. Set `paid_mask=0`, `has_asset_plan=true` on vault.
...
Logic: `sol_snapshot = vault_lamports.saturating_sub(rent_min)` (D2 — no AssetPlan read,
SOL is pure pro-rata); `started_at = now`; `sol_paid_mask = 0`. (Second call fails:
...
→ owner) and close `token_dist`. *Rent of TokenDist (payer-paid) returns to whoever
you choose — simplest: owner; or payer. Pick one and be consistent.*

### 4.9 `close_vault()` — permissionless (R9, S3, D5)
Accounts: `payer` · `owner: UncheckedAccount`(mut; must == `vault.owner`) ·
`vault_config`(mut, seeds, manual close) · `heartbeat_record`(mut, seeds) ·
`execution_log`(mut, seeds) · `asset_plan`(mut, seeds; required iff has_asset_plan) ·
`largest_benef`(for SOL dust). 
Guards: `require!(grace_elapsed && vault.executed && execution_log.completed)`;
`require!(execution_log.sol_paid_mask == full_mask_u32(len))`;
`require!(!vault.has_asset_plan || asset_plan.paid_mask == full)`;
**no residual token state** — caller must have closed all `TokenDist`/ATAs first (R9;
the program can't enumerate, so document the crank must run all `close_token_dist`
before `close_vault`; remaining SOL above rent is dust).
Logic: sweep SOL dust (`vault_lamports - rent_min` rounding remainder) → largest
beneficiary; **rent → owner** (D5) by closing VaultConfig/HeartbeatRecord/ExecutionLog/
AssetPlan to `owner`. Zero discriminators / realloc(0) per manual-close pattern.
...
`close_executed_vault`, `close_vault_ata` (folded into `close_token_dist`). Keep
`close_revoked_vault` / `close_executed_vault_by_owner` only as owner escape hatches if
still wanted.
...
for j in assignment order (grouped by mint, ascending index): if !paid(j) execute_specific_asset(j)
...
once `close_vault` succeeds (vault gone → existing auto-deregister handles it).
...
- [ ] ix: set_asset_plan, update_asset_plan, begin_execution, begin_token_dist,
      ...
      finalize_execution, close_token_dist, close_vault
...
## 13. New error codes (append to `errors.rs`)
`GraceNotElapsed`, `ExecutionAlreadyStarted`/`Finalized`, `AssetPlanRequired`,
...
- Token-2022 *deposit* support on withdraw/close (distribution via InterfaceAccount IS
  in v1; deposits can lag).
- Cranker fee-reimbursement from the vault (third-party keeper incentive).
