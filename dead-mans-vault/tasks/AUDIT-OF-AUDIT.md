...
> **Resolution (2026-07-05):** the doc-facing findings were **applied** — brief fixes A1–A4 + coverage gaps B1–B8 landed in `AUDIT-SCOPE.md` §5/§6; runbook fixes C1–C9 landed in `MAINNET-CUTOVER-RUNBOOK.md` (Helius-transport deploy, ~13 SOL budget, priority-fee/buffer-recovery, `--skip-new-upgrade-authority-signer-check` + deferred handoff as new Step 12.5, `solana-verify` + post-deploy bytecode check, cranker alerting, corrected rollback line) and `MAINNET-READINESS.md` (§1.2 budget, new §1.8 governance-rehearsal + §1.9 monitoring blockers). **D (test coverage) is a code/test change, not applied here** — it's scoped for the external auditor + a follow-up test PR. **E (defense-in-depth code hardening) is left for the auditor** (all items are safe today).
