...
test('dereg (WP6.1 LOW-fix): compound double-write fault (tombstone + durable marker both fail) → closed-vault proof SURVIVES (restart reconciles, no stale "enabled")', async () => {
  const o = makeOwner();
  const key = { cluster: CLUSTER, programId: PROGRAM, owner: o.owner, vault: deriveVault(o.owner) };
