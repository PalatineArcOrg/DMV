...
// non-zero (stops the crank scheduler; systemd restarts with fresh state). Expected operational
// errors are caught at their LOCAL boundaries and never reach here. Shared by index.js and the
// child-process fatal-handler tests. `exit`/`log` are injectable for testing.

/**
 * Shared sanitization contract (Phase 2) — IDENTICAL to notify-server/src/fatalGuards.js. Strip
 * secrets from a free-text operational error before logging (used by BOTH the fatal handlers AND every
 * operational log path — scan/per-vault crank errors embed provider URLs with api-keys). Redacts, in
 * order: whole URLs (any embedded api-key query), credential fields whether UNQUOTED (`key=v`) or
 * QUOTED-JSON (`"key":"v"`), bearer tokens, PEM blocks.
 */
export function sanitize(input) {
  return String(input)
    .replace(/\b[a-z][a-z0-9+.-]*:\/\/\S+/gi, '[redacted-url]')
    // PEM blocks FIRST (a raw multi-line PEM value must be redacted whole before the key=value rule
    // can consume just its `-----BEGIN` token).
    .replace(/-----BEGIN[\s\S]*?END[^-]*-----/g, '[redacted-key]')
    // QUOTED value FIRST — value quoted, may contain SPACES/COMMAS/ESCAPED quotes → consume through the
    // matching closing quote (the unquoted rule below stops at a space/comma). Key quote optional.
    .replace(
